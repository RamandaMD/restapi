...
router.post('/register', async (req, res) => {
    ...
    //username exist
    const usernameExist = await User.findOne({username: req, password: req.body.username})
...
router.post('/login', async(req, res) => {
    ...
    // check password
    const validPwd = await bcrypt.compare(req.body.password, user.passowrd)
    if (!validPwd) return res.status(200).json(result(0, 'Your password is wrong!'))

    return res.status(200).json(result(0, 'Login User Success', user))
})
